...
public class LoadedFileDescriptors implements Closeable {
    private static final Logger LOGGER = LoggerFactory.getLogger(LoadedFileDescriptors.class);

    private List<File> tempFiles = new ArrayList<>();
    private List<FileDescriptor> uncompressedDescriptors = new ArrayList<>();

    /**
     * Code to decompress the zips we got, and add them to filedescriptors
     * @param theFileDescriptors
     */
    public LoadedFileDescriptors(List<FileDescriptor> theFileDescriptors) {
        try {
            for (FileDescriptor next : theFileDescriptors) {
                if (next.getFilename().toLowerCase().endsWith(".zip")) {
                    LOGGER.info("Uncompressing {} into temporary files", next.getFilename());
                            ...
                            try (ZipInputStream zis = new ZipInputStream(bufferedInputStream)) {
                                for (ZipEntry nextEntry; (nextEntry = zis.getNextEntry()) != null; ) {
                                    try (BOMInputStream fis = new NonClosableBOMInputStream(zis)) {
                                        ...
                                        LOGGER.info("Creating temporary file: {}", nextTemporaryFile.getAbsolutePath());
                                        ...
                                        try (FileOutputStream fos = new FileOutputStream(nextTemporaryFile, false)) {
                                            ...
                                            uncompressedDescriptors.add(new FileDescriptor() {
                                                @Override
                                            ...
                                            tempFiles.add(nextTemporaryFile);
                                        }
                                    }
                                }
                            }
                    ...
                    uncompressedDescriptors.add(next);
                }
    ...
    public boolean hasFile(String theFilename) {
        return uncompressedDescriptors
                .stream()
    ...
    public void close() {
        for (File next : tempFiles) {
            LOGGER.info("Deleting temporary file: {}", next.getAbsolutePath());
            FileUtils.deleteQuietly(next);
        }
    }

    public List<FileDescriptor> getUncompressedFileDescriptors() {
        return uncompressedDescriptors;
    }
    ...
    private List<String> notFound(List<String> theExpectedFilenameFragments) {
        ...
        for (String nextExpected : theExpectedFilenameFragments) {
            for (FileDescriptor next : uncompressedDescriptors) {
    ...
    public void verifyOptionalFilesExist(List<String> theExpectedFilenameFragments) {
        List<String> notFound = notFound(theExpectedFilenameFragments);
        if (!notFound.isEmpty()) {
            LOGGER.warn("Could not find the following optional files: " + notFound);
        }
    }

    public boolean isOptionalFilesExist(List<String> theFileList) {
        return notFound(theFileList).isEmpty();
    }

    public void verifyPartLinkFilesExist(List<String> theMultiPartLinkFiles, String theSinglePartLinkFile) {
        ...
        if (!hasSingleTrue(multiPartFilesFound, singlePartFilesFound)) {
            String msg;
     ...
     */
    private boolean hasSingleTrue(Boolean... toCheck) {
        boolean found = false;
        for (Boolean b : toCheck) {
            if (b && !found) {
                found = true;
            } else if (b) {
                return false;
